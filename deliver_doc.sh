...
if ! rclone --config ./rclone.conf copy "${directory}/" "openpaperwork:documentation/libinsane/${out_name}" ; then
  echo "rclone failed"
  exit 1
fi

if ! rclone --config ./rclone.conf sync "${directory}/" "openpaperwork:documentation/libinsane/latest" ; then
  echo "rclone failed"
  exit 1
fi
